<template>
    ...
    >
        <img
            ...
            @click="openImage(index, image.id)"
        />
    </div>
    <Modal
        :show="showImageModal"
    ...
    >
        ...
        <svg
            ...
            fill="currentColor"
            viewBox="-1 -1 19 19"
            @click="downloadImage(clickedImage)"
        >
            <path
    ...
    methods: {
        async loadFolder() {
            if (
                ...
                } = await useFetch('/api/listImages?id=' + this.folderId)
                if (files) {
